      ...
      try {
        ...
        while (page <= pageCount) {
          const response = await fetch(
            `${strapiUrl}/api/restaurants?pagination[page]=${page}&pagination[pageSize]=100`,
            {
              method: "GET",
              headers,
            }
        ...
        const transformedRestaurants = allRestaurants
          .map((item: any) => ({
            ...
            deliveryTime: item.delivery_time || item.deliveryTime || "30-40 min",
        ...
        console.log(
          `✅ Pobrano ${transformedRestaurants.length} restauracji (z ${allRestaurants.length} total)`
        );
      } catch (err) {
        const errorMessage =
